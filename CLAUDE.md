...
コミットメッセージに **`[no-discord]`** を入れると、Discord投稿をせずサイトだけ作り直す。
公開後に表示の不具合を直して出し直すとき、クリエイターへの二重通知を避けるために使う。
...
`input/` に置くファイルは **`.xlsx` / `.xls` / `.csv` / `.tsv` のいずれでもよい**
(`weekly_picks.read_table` が拡張子で読み分ける。CSVは UTF-8/BOM付き/CP932 を自動判定)。
ファイル名の `Kalodata_Product_*` / `Kalodata_Video_*` という接頭辞は `--auto` の
判別に使うので**拡張子を変えても接頭辞は維持すること**。
列名さえ揃っていればエクスポートでも画面取得でも生成結果は同一になる
(9/7分で検証済み)。画面から取る場合の手順・必要列は `docs/kalodata-screen-capture.md`。
...
- 🎁 **自社サンプル可** (オレンジ): 案件リスト (`tap_list.csv` / `own_list.csv`) と商品ID一致
  (同名別IDは正規化商品名でフォールバック) した商品に自動付与。`CONFIG["own_sample_brands"]` の
  ブランド名部分一致でも付与される (こちらは現在空)。🔰/💎と独立判定で併記される
- **個別の例外は `CONFIG["own_overrides"]` (商品ID→指定) に書く**。`None` なら案件リストに
  載っていても🎁を付けず遷移先も差し替えない (取り下げ)、URLならリストに無い商品でも
  🎁を付けてそのリンクに差し替える。案件リストのCSVはGoogleシートのスナップショットで
  再同期すると上書きされるため、**CSVを直接編集せず必ずここに書くこと**
...
- **用途 (2026-08以降)**: 🎁自社サンプル可バッジの付与と、売れ筋/新商品カードの遷移先を
  ...
  正規化商品名でフォールバック
- **リンク差し替えの優先順位 (2026-09〜)**: `tap_list.csv` (TAP一覧) → `own_list.csv` の順に見て、
  **先に一致した方のアフィリエイトリンクを採用する**。🤝タブに載っている商品は必ずその
  TAP案件のリンクに揃えるため。両方に載っている商品はTAP側のリンクになる
  (`weekly_data_v1.apply_own_list` に優先度順のリストを渡す形。順序を変えたい場合はここ)
...
- **Kalodataへの自動ログイン/スクレイパーをこのリポジトリに実装しない**。
  データの取得元は「エクスポート(手動)」または「Claude in Chrome で画面から取得」の
  どちらでもよい (2026-09にユーザー判断で画面取得を許可) が、**取得はセッション側の作業**で、
  ログイン情報を持つ常設スクリプトはここに置かない。
  画面取得の手順と必要な列は `docs/kalodata-screen-capture.md` を参照
