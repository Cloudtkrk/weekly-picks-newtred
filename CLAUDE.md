...
- **同期方法**: ユーザーが「リスト同期して」等と言ったら、Google Driveコネクタの
  ...
  `own_list.csv` (列: カテゴリ/ショップ/商品名/価格/報酬率/アフィリエイトリンク/商品ID/ライブ) を
  再生成してコミット。`read_file_content` は大きいシートで途中までしか返らないので使わないこと。
...
- 生成時の動作 (`weekly_data_v1.py` / `report_html.py`):
  ...
  4. `live.html` (📺ライブタブ): CSVの**「ライブ」列に値がある商品**だけを集めたページ。
     列が空/無い週はページもタブも生成されない。カードには 📺LIVEタイムセール可能 ピルが付く
