...
CONFIG = {
    ...
    # --- チャレンジ(新商品): 掲載からこの日数以内のみ (エクスポート条件は掲載30日以内・売上50万円+) ---
    "challenge_max_listed_days": 30,
    ...
    # --- 🎁自社サンプル可の手動オーバーライド (商品ID → 指定) ---
    #     None を入れると、案件リストに載っていても 🎁 を付けず遷移先も差し替えない
    #     URL を入れると、リストに無い商品でも 🎁 を付けてそのリンクに差し替える
    #     tap_list.csv / own_list.csv はGoogleシートのスナップショットで再同期すると
    #     上書きされるため、個別の例外はCSVではなくここに書くこと
    "own_overrides": {
        # TYMO CURLPRO: 自社サンプル対象から取り下げ (2026-09-07 ユーザー指示)
        "1734578993180607721": None,
        # R7.栃木県産 とちぎの星 精米: 自社サンプル可に追加 (2026-09-07 ユーザー指示)
        "1736665753631032962": "https://affiliate.tiktok.com/api/v1/share/ALOIAZgdGUmb",
    },
    # --- 閲覧ゲート (公式LINE登録者限定) ---
